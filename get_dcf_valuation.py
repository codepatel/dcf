...
def get_dcf_df(df_dict={}, handler_data=[], rgr_next='5', opm_next='10', 
    ...
    for period in range(1, TERMINAL_YEAR_LENGTH+2):
        ...
        dcftable['EBIT($)'].append(dcftable['Revenue($)'][period] * dcftable['Operating Margin(%)'][period])
        dcftable['Net Income($)'].append(dcftable['EBIT($)'][period] * (1 - dcftable['Tax Rate(%)'][period]))
        ...
        dcftable['FCF($)'].append(dcftable['Net Income($)'][period] - dcftable['Reinvestment($)'][period])
